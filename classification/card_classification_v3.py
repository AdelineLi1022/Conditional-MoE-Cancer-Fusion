                ...
                for epoch in range(config.diffusion.aux_cls.n_pretrain_epochs):
                    for feature_label_set in train_loader:
                        if config.data.dataset == "gaussian_mixture":
                            ...
                            # ========= 修改1：数据集读取三元组 label, bag, rna =========
                            y_labels_batch, x_batch, rna_batch = feature_label_set
        ...
        if not self.args.train_guidance_only:
            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for i, feature_label_set in enumerate(train_loader):
                    ...
                    # ========= 修改2：MoE传入rna特征，读取融合fuse_emb =========
                    output, joint_losses = self.cond_pred_model(x_unflat_batch.to(self.device), rna_batch.to(self.device))
            
                    y_0_hat_batch = output['e_Y_prob']
                    # 替换原生expert_feature，使用融合特征fuse_emb
                    fuse_emb = output['fuse_emb']
                    fuse_emb = fuse_emb.unsqueeze(0)
                    ...
                    # 扩散模型输入改为融合特征fuse_emb
                    output = model(fuse_emb.to(self.device), y_t_batch, t, y_0_hat_batch)
                ...
                if epoch % self.config.training.validation_freq == 0 \
                    ...
                    if config.data.dataset == "toy":
                        ...
                        for test_batch_idx, feature_label_set in enumerate(test_loader):
                            # ========= 修改1：验证集读取三元组 =========
                            target, images, rna_batch = feature_label_set
                            ...
                            with torch.no_grad():
                                # ========= 修改2：MoE传入rna，读取融合fuse_emb =========
                                images = images[None,:,:]
                                output, joint_losses = self.cond_pred_model(images.to(self.device), rna_batch.to(self.device))
                                ...
                                label_t_0 = p_sample_loop(model, fuse_emb, target_pred, y_T_mean,
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for step, feature_label_set in tqdm(enumerate(test_loader)):
                ...
                # ========= 修改2：MoE传入rna，读取融合fuse_emb =========
                output, joint_losses = self.cond_pred_model(x_batch.to(self.device), rna_batch.to(self.device))
                target_pred = output['e_Y_prob']
                fuse_emb = output['fuse_emb']
