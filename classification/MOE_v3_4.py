...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        # ===================== 新增RNA门控融合模块 =====================
        # RNA自编码器：输入200维筛选后的差异基因，输出256维分子嵌入
        self.rna_encoder = nn.Sequential(
            nn.Linear(200, 512),
            nn.GELU(),
            nn.Linear(512, 256)
        )
        # 图像投影：512维expert_feature统一到256维
        self.img_proj = nn.Linear(512, 256)
        # 门控权重生成网络
        self.gate_net = nn.Linear(256 * 2, 256)
        # =================================================================


    # 新增rna_feat入参
    def forward(self, hidden_states: torch.Tensor, rna_feat=None) -> torch.Tensor:
        """ """
        ...
        if rna_feat is not None:
            # RNA编码到256维
            z_rna = self.rna_encoder(rna_feat)
            # 图像512→256
            z_img_proj = self.img_proj(expert_feature)
            # 拼接计算门控
            concat_emb = torch.cat([z_img_proj, z_rna], dim=-1)
            gate = torch.sigmoid(self.gate_net(concat_emb))
            # 动态加权融合
            fuse_emb = gate * z_img_proj + (1 - gate) * z_rna
        else:
            # 无RNA，只用投影后的图像特征
            fuse_emb = self.img_proj(expert_feature)
        # 把融合特征写入返回字典
        results_dict["fuse_emb"] = fuse_emb
        # ============================================================
